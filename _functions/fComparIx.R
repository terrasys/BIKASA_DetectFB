...
o[which(o$FBID=="DESTLI0500950004"),]
head(o)
#-------------------------------------------------------------------------------
...
df.columnnames <- c("IDFB","AreaFB","AreaFBST","CI.MBI1","DT.MBI1","CI.MBI2","DT.MBI2","CI.K","DT.K","CI.US","DT.US","CI.UL","DT.UL","CI.LS","DT.LS","ST","FBID")
...
for(i in 1:length(o.fb)){
  ...
  #Geoherkunft
  df.CI[i,16] <- as.character(unique(o.fb[[i]]$GEOHERK_ST))
  #FBID
  df.CI[i,17] <- as.character(unique(o.fb[[i]]$FBID))
  setTxtProgressBar(pb, i)
}
...
fb$CI_SL <- (fb$CI_LS+fb$CI_K+fb$CI_MBI1)/3
head(fb)
fb[which(fb$FBID=="DESTLI0500950004"),]
##Boxplot of 
#setwd(file.path(W.DIR,OUT.DIR))
...
fb <- st_read(paste(W.DIR,OUT.DIR,substr(FB.SHP, start=1, stop=(nchar(RO.SHP)-4)),"_Feldblockbewertung",c(".shp"),sep=""))
head(fb)
#head(df.CI)
#-------------------------------------------------------------------------------
...
df.CI.E <- fb[which(fb$CI_SL>=V.TH),]
#selection of parcels with a certain overlap  
df.CI.E <- df.CI.E[which((df.CI.E$AreaFBST/10000)>=V.HA),]
...
dev.off()
  ...
  rf.Fit <-   train(CI_SL ~ CI_K + CI_MBI1 + CI_MBI2 + CI_UL + CI_US + ST,
                    data = df.CI.E,
                    ntree=1000,
                    method = "rf",
                    trControl = ctrl,
  ...
  for(k in 1:length(df.CI.ST)){
    ...
    set.seed(123)
    ctrl <- trainControl(method="repeatedcv",number=5)
    rf.Fit <-   train(CI_SL ~ CI_K + CI_MBI1 + CI_MBI2 + CI_UL + CI_US,
                      data = df.CI.ST[[k]],
                      ntree=1000,
                      method = "rf",
                      trControl = ctrl,
    ...
    sink()
    #interaction
    df.CI.ST[[k]]$I.CI <- interaction(as.factor(round(df.CI.ST[[k]]$CI_K/10,0)),
                                      as.factor(round(df.CI.ST[[k]]$CI_US/10,0)), 
                                 as.factor(round(df.CI.ST[[k]]$CI_UL/10,0)),
                                 as.factor(round(df.CI.ST[[k]]$CI_MBI1/10,0)),
                                 as.factor(round(df.CI.ST[[k]]$CI_MBI2/10,0)),
                                 sep="")
    head(df.CI.ST[[k]])
    ...
    Count <- na.omit(Count)
    df.CI.ST[[k]]$Count <- 1
    Area  <-as.data.frame(aggregate(df.CI.ST[[k]]$AreaFBST, by=list(df.CI.ST[[k]]$I.CI),FUN=sum, na.rm=TRUE))
